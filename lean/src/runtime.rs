...
/// Callers must either avoid initializing the Lean runtime multiple times, or
/// must use runtime components that are safe to initialize multiple times.
pub unsafe fn run_in_lean_sync_runtime_unchecked<
...
> {
    let runtime =
        { unsafe { ThreadRuntimeImpl::new_primary_thread(modules_initialization_error_handler) } }
            .map_err(LeanError::Initialization)?;
    let value = run(&runtime)?;
    Ok(value)
}
...
/// # Panics
///
/// Panics if this function or [`run_in_lean_sync_runtime()`] have already been
/// called. The runtime is single-use to eliminate overhead from repeatedly
...
/// # Panics
///
/// Panics if this function or [`run_in_lean_runtime()`] have already been
/// called. The runtime is single-use to eliminate overhead from repeatedly
/// checking whether it has already been initialized. There is no need to call
/// this function multiple times in the same program.
///
/// See also [`run_in_lean_sync_runtime_unchecked()`] which does not panic but
/// delegates repeated initialization checks to the caller.
pub fn run_in_lean_sync_runtime<
    C: SyncRuntimeComponents,
    M: Modules,
    T,
    ModulesInitializationError: Error,
...
> {
    ...
    ONCE_INITIALIZATION_GUARD.call_once(|| {
        result = Some(unsafe {
            run_in_lean_sync_runtime_unchecked(modules_initialization_error_handler, run)
        });
    });
    result.expect("attempt to reuse the Lean runtime. The runtime is single-use to eliminate overhead from repeatedly checking whether it has already been initialized")
}
...
/// [`run_in_lean_runtime()`] or [`run_in_lean_sync_runtime()`] have already
