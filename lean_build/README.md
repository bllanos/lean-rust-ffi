...
As described in the [main README](../README.md#unsafe-rust), this crate contains utilities for writing the [build scripts](https://doc.rust-lang.org/cargo/reference/build-scripts.html) of `*-sys` crates that use Lean and Lean libraries.
...
`lean-build` uses a version of Lean already present on their system. Users can specify the Lean version using the same methods that can be used to specify the Lean version when building an ordinary Lean package. In other words, `lean-build` intends to follow the same conventions as the Lean community for version management.

There are several implications of this approach:

1. `lean-build` depends on `bindgen` to create Rust bindings for arbitrary versions of Lean at build time, and therefore requires that [`bindgen`'s own dependencies are installed](https://rust-lang.github.io/rust-bindgen/requirements.html).
2. Some versions of Lean may be so different from those tested with `lean-build` that they may be incompatible with the crate. In particular, `lean-build` assumes the existence of some [Lean runtime functions](https://lean-lang.org/doc/reference/latest/Run-Time-Code/Foreign-Function-Interface/#ffi-initialization) whose declarations are not in Lean's public C/C++ header file, and therefore cannot be processed by `bindgen`. See [`src/lean_sys_root_module.rs`](src/lean_sys_root_module.rs).
...
For simplicity, there is no intention to support both static and dynamic linking. To use dynamic linking, one can do either of the following:
